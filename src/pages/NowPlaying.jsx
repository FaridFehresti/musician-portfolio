...
const SLEEVE = 340 // px — big holographic card

export default function NowPlaying() {
  const {
    currentTrack, isPlaying, isPaused,
    currentTime, duration, howl,
  ...
  return (
    ...
    >
        ...
        >
          <svg viewBox="0 0 900 900">
            {[420, 380, 340, 300, 260, 220, 180, 140, 100].map(r => (
              <circle key={r} cx="450" cy="450" r={r} fill="none" stroke="rgba(0,229,255,0.6)" strokeWidth="0.8" />
            ))}
          </svg>
        </motion.div>
      </div>
      ...
      <div
        ...
        style={{
          background: `radial-gradient(ellipse 55% 45% at 50% 38%, rgba(255,47,208,${glowAlpha}) 0%, transparent 68%)`,
      ...
      <div className="relative z-10 flex flex-col items-center gap-10 w-full max-w-2xl px-4">
        ...
        <div className="w-full flex flex-col gap-2">
          <div
            style={{ position: 'relative', height: 4, borderRadius: 2, background: 'rgba(243,233,255,0.1)', cursor: 'pointer' }}
            onClick={e => {
        ...
        <div className="flex items-center justify-center gap-8 w-full">
          ...
          <motion.button
            onClick={togglePlay}
            whileHover={{ scale: 1.08, backgroundColor: 'rgba(255,47,208,0.24)' }}
            whileTap={{ scale: 0.94 }}
...
function QueueList({ queue, queueIndex, loadTrack }) {
  ...
  return (
    <div className="w-full">
      ...
      <div className="rounded-xl overflow-hidden" style={{ background: 'var(--color-surface)', border: '1px solid rgba(122,77,255,0.14)' }}>
        {upcoming.map((track, i) => (
          <button
            ...
            style={{
              borderBottom: i < upcoming.length - 1 ? '1px solid rgba(243,233,255,0.05)' : 'none',
              background: 'transparent', cursor: 'pointer',
            }}
            onMouseEnter={e => (e.currentTarget.style.background = 'rgba(255,47,208,0.06)')}
